...
describe("wallet authentication service", () => {
  ...
  it("sends only the wallet address for a SIWE challenge and never asserts a role", async () => {
    ...
    expect(mocks.post).toHaveBeenNthCalledWith(2, "/auth/verify", {
      message: "synthetic SIWE message",
  ...
  it("fails locally when no compatible injected wallet exists", async () => {
    await expect(connectWalletAndSignIn()).rejects.toThrow(
      "Install MetaMask, Rabby, or another compatible browser wallet",
    );
    expect(mocks.post).not.toHaveBeenCalled();
  });

  it("refreshes and clears the in-memory CSRF token with the session", async () => {
